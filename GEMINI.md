...
- **`TeamCulture.vue` Bilibili 视频集成:**
  - **问题:** 需要将招新宣传片从本地视频文件替换为在线 Bilibili 视频，并将其集成到 `main` 分支的组件中。
  - **方案:**
    1.  使用 Bilibili 官方提供的 `<iframe>` 嵌入代码替换了 `<video>` 标签。
    2.  为 `<iframe>` 添加了一个响应式 CSS 容器，利用 `padding-top: 56.25%` 技巧确保视频在所有屏幕尺寸下都能保持 16:9 的宽高比，避免变形。
    3.  根据用户指示，将此视频模块**手动添加**到 `main` 分支版本的 `TeamCulture.vue` 文件末尾，保留了 `main` 分支独有的画廊交互逻辑，避免了直接覆盖文件导致的逻辑回退。
