...
- **`TeamCulture.vue` 布局 Bug 修复：**
  - **问题**：修复了一个偶发的竞态条件（Race Condition）Bug。该 Bug 导致 `TeamCulture` 组件有时会在其内部图片完全加载前就计算了自身高度，从而获得一个错误的高度值。这会引发两个问题：1. 组件无法撑开应有的空间，导致与下方的 `TeamMembers` 组件内容重叠。2. 滚动劫持动画因错误的滚动距离计算而失效。
  - **方案**：重构了组件的挂载逻辑。创建了一个新的 `waitForImagesAndSetHeight` 函数，该函数会等待当前视图内的所有 `<img>` 标签加载完成后，再调用 `setHeight` 函数来设置容器的最终高度。此逻辑被应用于 `onMounted` 钩子和切换章节的 `watch` 监听器中，确保了布局计算的可靠性。
  - **结果**：彻底解决了内容重叠和动画失效的问题，增强了页面的稳定性和用户体验。

- **`TeamMembers.vue` 成员列表重构与增强：**
  - **问题**：需要为一批只有姓名和去向、没有照片的往届队员，在现有设计风格的基础上增加一个展示区域。
  - **方案演进**：
    1.  **初步方案 (已废弃)**：在组件底部新增一个独立的“亦有群星，散落四方”区域，用多列文本列表展示。此方案因视觉上与主组件割裂而被放弃。
    2.  **整合方案 (最终方案)**：将无照片的队员无缝整合进现有的筛选系统和卡片网格布局中，实现了更统一的交互体验。
